# Security Playbook: Laravel 13 DDD / lexio
...
>
> Область применения: разделы про PostgreSQL/Redis/Kafka/Soketi/Horizon относятся к будущей прод-инфраструктуре — в lexio backend их сейчас нет (sqlite, sync/array-драйверы). Читать как ориентир при переезде, чек-лист после сессий — по CHECK_SECURITY.md.
...
> Технические меры. Юридическую квалификацию (какой режим данных применим — 152-ФЗ, GDPR и т.д.) должен подтвердить юрист/DPO проекта — этот раздел не заменяет такую оценку. В lexio нет медицинских данных; чувствительные данные — учебный профиль и контакты для входа.

### 25.1 Классификация данных

Перед реализацией любой новой фичи с персональными данными — явно промаркировать поля:

| Класс | Примеры в lexio | Требования |
|---|---|---|
